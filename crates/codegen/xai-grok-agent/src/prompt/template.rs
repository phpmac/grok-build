...
//!
//! Templates are plain Markdown files under `templates/`, loaded directly via
//! `include_str!` at compile time. Edit the `.md` files to change prompt text.

use zeroize::Zeroizing;

/// The base prompt template, loaded from `templates/prompt.md`.
pub(crate) fn base_template() -> Zeroizing<String> {
    Zeroizing::new(include_str!("../../templates/prompt.md").to_string())
}
...
pub(crate) fn apply_patch_template() -> Zeroizing<String> {
    Zeroizing::new(include_str!("../../templates/apply_patch_prompt.md").to_string())
}
...
/// The subagent-specific base template, loaded from `templates/subagent_prompt.md`.
pub(crate) fn subagent_template() -> Zeroizing<String> {
    Zeroizing::new(include_str!("../../templates/subagent_prompt.md").to_string())
}
...
mod tests {
    ...
    /// Verify the plaintext templates load correctly and render with resolved
    /// interpolations (no raw `${{ }}` tokens survive rendering).
    #[test]
    fn test_plaintext_templates_render_fully() {
        let renderer = default_renderer();
        let placeholders = default_placeholders();

        let base = render_base(&renderer, &placeholders);
        assert!(
            base.contains(crate::prompt::context::DEFAULT_SYSTEM_PROMPT_LABEL),
            "base template must resolve system_prompt_label"
        );
        assert!(!base.contains("${{"), "base template: no unresolved variables");
        assert!(!base.contains("${%"), "base template: no unresolved blocks");

        let apply_patch = render_apply_patch(&renderer, &placeholders);
        assert!(
            !apply_patch.contains("${{"),
            "apply-patch template: no unresolved variables"
        );
        assert!(
            !apply_patch.contains("${%"),
            "apply-patch template: no unresolved blocks"
        );

        let subagent = render_subagent(&renderer, &placeholders);
        assert!(
            !subagent.contains("${{"),
            "subagent template: no unresolved variables"
        );
        assert!(
            !subagent.contains("${%"),
            "subagent template: no unresolved blocks"
        );
    }
