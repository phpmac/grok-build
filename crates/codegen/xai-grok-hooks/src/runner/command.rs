...
pub async fn run_command_hook(
    ...
    match result {
        ...
        Ok(Ok(output)) => {
            ...
            match mode {
                GateKind::Observe => {
                    // PostToolUse 等: 仍解析 soft-warn JSON, 否则审计 reason 被丢弃.
                    if exit_code == 0 {
...
fn parse_blocking_result(
    ...
    if let Some(output) = json_decision {
        match gate_json_to_decision(output, hook_name) {
            ...
            Ok(decision @ HookDecision::Allow { .. }) => {
    ...
    match exit_code {
        0 => (HookRunnerResult::Decision(HookDecision::Allow { additional_context: None }), elapsed),
        GATE_EXIT_CODE => (
