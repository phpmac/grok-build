...
- **同步全局规则到系统提示词 prompt.md**: mindset 增补更优方案强制一句建议, 以及出乎意料的改动先调查再改且须用户同意, 不确定先问; action_safety 把未授权禁 commit 提到独立条.
...
- **同步上游 monorepo 1.0.24** (`SOURCE_REV` eb4a894d): 多文件夹 `grok-workspaced` daemon; 调度器唤醒交互 (fix/delete/update scheduled task); 子代理生命周期按 attempt 跟踪且完成通知含输出; edit_file diff 锚定真实文件行号; auto 模式 git 例行操作 fail-closed 允许列表; folder trust 门控 project instructions/skills 与目录型 vendor hook 路径; TUI 启动上报与启动时序拆分; Shift+J/K 跳转 turn; 队列 follow-up 穿过等待保持; Esc 不再取消运行中 turn; `DISABLE_TELEMETRY` 生效且 test/dev 构建遥测不进生产目的地; Dashboard 统一头部; 会话选择器拆到 `session_picker_surface` 并重构 stacked 布局 (StackedColumn/LogoTier); 图片会话路径策略 (SessionPathPolicy) 与 textarea `get_range` 安全化; saturating_sub 加固.
- 本地设计保留不变 (关自动更新 / 启动 UI / soft-warn / language / 去 Sentry 等).

## Notes

- 产品版本继续走本地 1.x; 上游锁步号 1.0.24, SOURCE_REV 推进.
...
- **J/K** now jump between conversation turns using the same targeting as the timeline arrows, instead of snapping to assistant responses.
...
- **同步上游 monorepo 1.0.10** (`SOURCE_REV` 70ec060e): UserPromptSubmit 阻塞门与 hook 网关重构 (run prompt gate before chat-state commit, block 后 hold 队列); 身份戳 + runtime 从 chat store 重水合; auto 模式放行 mkdir/touch 安全创建; headless 会话默认 always-allow 不卡提示; websocket crates 统一 0.28 去重; sandbox 规范化 socket mask + Devbox 强制 bubblewrap; 旧模型 slug 重定向 grok-4.6; voice 在 Ubuntu 22.04 回退; computer-hub bot relay 连接管理; workflow 子代理限速余波与任务重试.

## Notes

- 本地设计保留不变: 关自动更新 / 启动 UI 精简 / soft-warn+hookify (上游重构 hook 网关, 本地 soft-warn 语义保留) / language / Tasks 面板位置 / 会话标题左对齐 / 无 Sentry / 明文提示词.
- 产品版本继续走本地 1.x; 上游锁步号 1.0.10.
...
- **输出默认只结论**: 禁止先长文/大表再末尾一句话; 未追问不展开; 约 100 字自然语言上限 (代码路径日志不计).

# 1.15.5 — 2026-08-07

## Features

- **输出语言规范** (`prompt.md`): 最终自然语言约 100 字内 (代码/路径/日志不计); 禁免责/安全合规废话; 禁中文标点与 emoji; 重点用 `>` 引用块; 进度条 `████░░░░`; 事实核查补强.

# 1.15.4 — 2026-08-07

## Features

- **系统提示词增强** (`templates/prompt.md`): Hooks 强制遵守 (任意插件名); 同类问题自查只汇报并询问; 协作边界 (用户做主/禁乱改/根因未立不修/禁宣称全解决); 领域预习 (先读项目角色职能, 领域任务先调研再写码); action_safety 禁装依赖与改系统底层.
- Claude.md 设计保留表同步记录上述本地定制.
...
- 产品版本继续走本地 1.x; 上游锁步号 0.2.116, SOURCE_REV 推进.
...
- **同步上游 monorepo 0.2.112 续** (`SOURCE_REV` 见 SOURCE_REV 文件): UI 立即显示并后台拉 models/settings; 大 session 加载峰值内存受限; fork 流式 inherited replay 限内存; subagent lifecycle soak; herdr 多路复用检测; plan 审批 y 复制全文; 源标签终端版本 telemetry 等.
- 本地设计保留不变 (关自动更新 / 启动 UI / soft-warn / language / 去 Sentry 等).
...
- **同步上游 monorepo 0.2.109** (`SOURCE_REV` 见 SOURCE_REV 文件): `/usage`, `/doctor`, model_providers, reasoning `max`, combine_queued_prompts, session 跨目录 resume, worktree 自动 GC, 安全门控等 (含 0.2.107/108).
- 本地设计保留不变 (关自动更新 / 启动 UI / soft-warn / language 等).

## Notes

- 产品版本继续走本地 1.x; 上游锁步号 0.2.109.
...
- **同步上游 monorepo 0.2.105** (`SOURCE_REV` 见 SOURCE_REV 文件): `/btw` minimal, `/summarize`, 默认 Grok 4.5, shell 环境与滚动等.
- 本地设计保留不变 (关自动更新 / 启动 UI / soft-warn / language 等).
