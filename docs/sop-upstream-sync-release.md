...
- **打 tag 前必须过完整编译验证** (v1.23.0 / v1.20.2 曾拿 Release 当第一次编译连挂): 先 push `main`, 等 `.github/workflows/precheck.yml` (`cargo check --locked -p xai-grok-pager-bin` + fork 回归 `cargo test`) 全绿, 才允许 push tag. 本机禁 cargo; 不要用 tag Release 试编译
